...
// class Size is used to define the dimensions of all walls in the game
// interface
class Size {
 public:
...
void prepInterface() { std::cout << "\033[2J"; }
...
// printWalls uses the Size as an input for printing the walls at the current
// cursor position line by line each \u25?? is a different ASCII symbol used in
// the wall
void printWalls(const Size& walls) {
  ...
  for (int i = 0; i < walls.height(); ++i) {
    std::cout << "\033[1B"
              << "\033[" + std::to_string(walls.width() + 2) +
                     "D";  // repositioning of the cursor to be underneath of
                           // the above line
    std::cout << "\u2551"
              << "\033[" + std::to_string(walls.width()) + "C"
              << "\u2551";
  }

  // print the last line of the wall
  std::cout << "\033[1B"
            << "\033[" + std::to_string(walls.width() + 2) + "D";
  std::cout << "\u255A";
  for (int i = 0; i < walls.width(); ++i) {
    std::cout << "\u2550";
  }
...
void moveUp() { std::cout << "\033[1A"; }

void moveDown() { std::cout << "\033[1B"; }

void moveRight() { std::cout << "\033[1C"; }

void moveLeft() { std::cout << "\033[1D"; }

void printCerquilha(int xOld, int yOld, int x, int y) {
  std::cout << "\033[0;0f";
  std::cout << "   x: " << x << "\t   y: " << y << std::endl;
  std::cout << "xOld: " << xOld << "\tyOld: " << yOld;
  std::cout << "\033[" + std::to_string(yOld) << ";" << std::to_string(xOld)
            << "f";
  std::cout << "o";
